<template>
  <div class="max-w-6xl mx-auto py-8 px-4 space-y-10 font-mont">
    ...
    <div class="mt-10">
      ...
      <p class="mt-3 text-sm text-gray-600">
        Đã chọn:
        <span class="font-semibold">{{ previewGallery.length }}</span>
        <span v-if="galleryError" class="text-red-600 ml-2">{{ galleryError }}</span>
        <span v-else-if="previewGallery.length > 0 && previewGallery.length % 6 === 0" class="text-green-600 ml-2">
          ✓ Hợp lệ
        </span>
      </p>
    ...
    <div class="mt-10">
      ...
      <p class="mt-3 text-sm text-gray-600">
        Đã chọn:
        <span class="font-semibold">{{ previewFloorPlan.length }}</span>
      </p>

      <div class="grid grid-cols-3 gap-4 mt-4">
        <div
          v-for="(img, idx) in previewFloorPlan"
          :key="idx"
          class="relative group w-full h-32 rounded-lg overflow-hidden shadow"
        >
          <img :src="img" class="object-cover w-full h-full" />
          <button
            @click.stop="removeFloorPlanImage(idx)"
            class="absolute top-2 right-2 bg-black/60 text-white rounded-full w-7 h-7 flex items-center justify-center opacity-0 group-hover:opacity-100 transition"
          >
            ✕
          </button>
...
if (props.product) {
  if (props.product.thumbnail) {
    previewThumbnail.value = props.product.thumbnail.startsWith("/storage/")
      ? props.product.thumbnail
      : `/storage/${props.product.thumbnail}`;
  }
...
// Watch gallery count for validation
watch(
  () => previewGallery.value.length,
  (count) => {
...
function removeThumbnail() {
  if (previewThumbnail.value && previewThumbnail.value.startsWith("blob:")) {
    URL.revokeObjectURL(previewThumbnail.value);
  }
  previewThumbnail.value = null;
...
function onFloorPlanChange(e) {
  const files = Array.from(e.target.files);
  for (const f of files) {
    previewFloorPlan.value.push(URL.createObjectURL(f));
    form.value.floor_plan.push(f);
  }
  e.target.value = null;
}

function removeFloorPlanImage(i) {
  if (previewFloorPlan.value[i] && previewFloorPlan.value[i].startsWith("blob:")) {
    URL.revokeObjectURL(previewFloorPlan.value[i]);
  }
  previewFloorPlan.value.splice(i, 1);
  form.value.floor_plan.splice(i, 1);
}

// Submit
async function handleSubmit() {
  // Validate gallery count
  if (form.value.gallery.length > 0 && form.value.gallery.length % 6 !== 0) {
